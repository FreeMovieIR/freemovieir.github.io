...
function createHomeSearchCard(item) {
  const title = item.title || item.name || 'نامشخص';
  const overview = item.overview ? `${item.overview.slice(0, 90)}...` : 'توضیحی موجود نیست';
  const type = item.media_type === 'tv' ? 'series' : 'movie';
  return `
    <div class="bg-gray-800 rounded-lg overflow-hidden">
      <img src="https://image.tmdb.org/t/p/w500${item.poster_path || ''}" alt="${title}" class="w-full h-64 object-cover">
      <div class="p-3">
        <h3 class="font-bold text-sm mb-2">${title}</h3>
        <p class="text-xs text-gray-300 mb-2">${overview}</p>
        <a href="/${type}/index.html?id=${item.id}" class="text-yellow-400 text-sm">مشاهده</a>
      </div>
    </div>
  `;
}

async function runHomeAjaxSearch(query) {
  const resultsContainer = document.getElementById('home-search-results');
  if (!resultsContainer) return;
  const q = query.trim();
  if (q.length < 2) {
    resultsContainer.innerHTML = '<p class="text-gray-400 col-span-full">حداقل دو کاراکتر وارد کنید.</p>';
    return;
  }

  resultsContainer.innerHTML = '<p class="text-gray-400 col-span-full">در حال جستجو...</p>';
  const url = proxify(`https://api.themoviedb.org/3/search/multi?api_key=${apiKey}&language=fa&query=${encodeURIComponent(q)}`);

  try {
    const res = await fetch(url);
    if (!res.ok) throw new Error('search failed');
    const data = await res.json();
    const items = (data.results || []).filter((i) => i.media_type === 'movie' || i.media_type === 'tv').slice(0, 12);

    if (!items.length) {
      resultsContainer.innerHTML = '<p class="text-gray-400 col-span-full">نتیجه‌ای پیدا نشد.</p>';
      return;
    }

    resultsContainer.innerHTML = items.map(createHomeSearchCard).join('');
  } catch (error) {
    resultsContainer.innerHTML = '<p class="text-red-400 col-span-full">خطا در جستجو. دوباره تلاش کنید.</p>';
  }
}
...
function setupHomeSearch() {
  ...
  button.addEventListener('click', () => runHomeAjaxSearch(input.value));
  input.addEventListener('keydown', (event) => {
    if (event.key === 'Enter') {
      window.location.href = `/search/index.html?q=${encodeURIComponent(input.value.trim())}`;
    }
  });
}

function updateSeoCrawlLinks(items = []) {
  const container = document.getElementById('seo-crawl-links');
  if (!container) return;
  ...
  const links = items
    ...
    .map((item) => {
      ...
      const typePath = item.media_type === 'tv' ? 'series' : 'movie';
      return `<a class="underline hover:text-white mr-3" href="/${typePath}/index.html?id=${item.id}">${title}</a>`;
    })
    .join('');

  container.innerHTML = links || '<p>لینکی برای نمایش وجود ندارد.</p>';
}
